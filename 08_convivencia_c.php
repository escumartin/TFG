...
var rol = AJAXCall("08_convivencia", "OPC=TIPO");

// RECOJO LAS COMUNIDADES
comu = AJAXCall("08_convivencia", "OPC=COMUNIDADES");
try {
    comunidades = eval(comu);
}
catch (e) {
    console.log("ERROR EUCARISTIA COMUNIDADES:\n\n" + comu);
}

// RECOJO LOS LUGARES
lug = AJAXCall("08_convivencia", "OPC=LUGARES");
try {
    lugares = eval(lug);
}
catch (e) {
...
var admin = AJAXCall("08_convivencia", "OPC=ADMIN");

if (admin) {
    NC = $2("numeroComunidad").value;
    respuestaComu = AJAXCall("08_convivencia", "OPC=CONVIVENCIAS&NC=" + NC);
    try {
    ...
    // RECOJO LAS CONVIVENCIAS
    respuesta = AJAXCall("08_convivencia", "OPC=CONVIVENCIAS");
    try {
        resp = eval(respuesta);
    }
    catch (e) {
        console.log("ERROR CONVIVENCIA:\n\n" + respuesta);
    }
}


// <div class="card border-dark m-2">
//             <div class="card-header bg-warning border-dark">Convivencia</div>
//             <div class="card-body bg-warning">
//                 <h5>Convivencia de domingo</h5>
//                 <p><strong>Lugar:</strong> Salón de la 1ª</p>
//             </div>
//             <div class="card-footer bg-warning border-dark">2019-03-30 21:00:00</div>
//         </div>

// 3,Convivencia de transmisión 2019,110,Convivencia de transmisión,16,2019-11-21 21:00:00,Puerto lumbreras
...
for (i = 0; i < resp.length; i++) {
    ...
    // PRIMER DIV
    div1.setAttribute("class", "card-header bg-warning border-dark");
    div1.innerHTML = resp[i][3];

    // SEGUNDO DIV
    div2.setAttribute("class", "card-body bg-warning");
    ...
    // TERCER DIV
    div3.setAttribute("class", "card-footer bg-warning border-dark");
    ...
    if (rol) {
        ...
        bEdit.setAttribute("class", "btn btn-secondary large material-icons bg-warning");
        bEdit.innerHTML = "edit";
        bDel.setAttribute("class", "btn btn-secondary large material-icons bg-warning");
        ...
        bEdit.onclick = function (e) {
            ...
            ID_CONVIVENCIAS = resp[id][0];
            $2("letrero").innerHTML = "Actualizar convivencia";
            $2("nuevaConvivencia").style.display = 'none';
            $2("actualizarConvivencia").style.display = '';
            $2("opcion").value = "ACTUALIZAR";
            $2("idEvento").value = ID_CONVIVENCIAS;
            $2("activadorP").click();

            var respuestaConvivencia = AJAXCall("08_convivencia", "OPC=DATOS_ACTUALIZAR&idEvento=" + ID_CONVIVENCIAS);
            try {
                var resp2 = eval(respuestaConvivencia);
            }
            catch (e) {
...
if (rol) {
    ...
    button.setAttribute("data-target", "#myModal-añadirConvivencia");

    button.onclick = function () {
        $2("formConvivencia").reset();
        $2("opcion").value = "INSERTAR";
        $2("nuevaConvivencia").style.display = '';
        $2("actualizarConvivencia").style.display = 'none';
        $2("letrero").innerHTML = "Añadir convivencia";
    }
    ...
    // BOTON AÑADIR
    $2("nuevaConvivencia").onclick = function () {
        $2("formConvivencia").onsubmit = function () {
            ...
            if ($2("fechaFinal").value < $2("fechaInicio").value) {
                ...
                insertar = AJAXCall("08_convivencia", "formConvivencia");
                if (insertar) {
                    $2("btn-cerrar2").click();
    ...
    $2("modalEliminar").onclick = function () {
        Eliminar = AJAXCall("08_convivencia", "OPC=ELIMINAR&idEvento=" + ID_CONVIVENCIAS);
        if (Eliminar) {
            GetModule("08_convivencia", "main");
        } else {
            alert("ERROR, al eliminar convivencia");
        }
    }

    // BOTON ACTUALIZAR
    $2("actualizarConvivencia").onclick = function () {
        $2("formConvivencia").onsubmit = function () {
            event.preventDefault();
            $2("opcion").value = "ACTUALIZAR";
            $2("idEvento").value = ID_CONVIVENCIAS;
            if ($2("fechaFinal").value < $2("fechaInicio").value) {
                ...
                var actualizar = AJAXCall("08_convivencia", "formConvivencia");
                if (actualizar) {
                    $2("btn-cerrar2").click();
                    GetModule("08_convivencia", "main");
                } else {
